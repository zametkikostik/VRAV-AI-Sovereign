"""
Anti-Hallucination Guard (strengthened)

Layers:
  1. Factual-claim detection
  2. RAG grounding against retrieved skills / tool context
  3. Optional web search verification (Serper / DDG)
  4. Confidence + rewrite when unsupported
"""
...
class AntiHallucinationGuard:
    ...
    async def validate_response(
        ...
        # 1. RAG grounding score against provided evidence
        ...
        # 3. External fact check when risky
        ...
        if grounding_docs and grounding_score < self.rag_min_grounding and fact_check.confidence < 0.7:
            final_response = (
                f"{response}\n\n"
                f"⚠️ [VRAV Anti-Hallucination] Low grounding to retrieved skills/sources "
            ...
            final_response = (
                f"{response}\n\n"
                f"⚠️ [VRAV Fact-Check] Confidence: {fact_check.confidence:.0%}. "
    ...
    async def _grounding_score(
        ...
        if not corpus:
            return 0.5  # neutral when no evidence provided
        try:
            ...
            logger.debug("grounding score failed: %s", e)
    ...
    def _looks_factual(self, text: str) -> bool:
        patterns = [
            r"\d{4}",
            r"\d+%",
            r"закон|законът|директива|регламент",
            r"чл\.|член\s+\d+",
            r"€\s*\d+|\d+\s*лв",
            r"България|Европейски\s+съюз|ЕС\b",
            r"according to|съгласно|по данни",
        ]
    ...
    async def _fact_check(
        self, entities: List[str], context: Dict[str, Any]
    ) -> FactCheckResult:
        if not entities:
        ...
        if not sources and not self.serper_key:
            # Use grounding from context as soft evidence
            if context.get("tool_context") or context.get("grounding_docs"):
                return FactCheckResult(
                    ...
                    notes="Verified against tool/RAG context (no external search key)",
                )
            return FactCheckResult(
                verified=False,
                confidence=0.55,
                entities_checked=entities,
                sources=[],
                notes="No search API key — limited verification",
            )
        ...
        return FactCheckResult(
            ...
            entities_checked=entities,
            sources=sources,
            notes="Cross-checked via web search (Bg/EU priority)",
        )
