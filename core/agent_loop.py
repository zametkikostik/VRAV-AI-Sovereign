"""
ReAct-style tool-calling loop with true token streaming (Ollama stream: true).

During each think round we stream tokens live. If the completed reply contains a
tool_call, we emit tool_call/tool_result and continue. Otherwise we treat the
streamed text as the final answer (already delivered as token events).
"""
...
from core.streaming.ollama_stream import stream_ollama
...
MAX_TOOL_ROUNDS = getattr(settings, "max_tool_rounds", 6)
...
class AgentToolLoop:
    ...
    async def _stream_think(
        ...
        """Yield text chunks from Ollama stream:true; caller assembles full reply."""
        async for chunk in stream_ollama(messages, model, temperature, base_url=self.ollama_url):
            yield chunk
    ...
    async def run_stream(
        ...
        """SSE: status, think_token, tool_call, tool_result, token, done."""
        ...
        yield sse_event("status", {"phase": "agent_loop", "model": model, "stream": True})
        ...
        for round_i in range(MAX_TOOL_ROUNDS):
            ...
            buf: List[str] = []
            try:
                async for chunk in self._stream_think(messages, model=model):
                    buf.append(chunk)
                    yield sse_event("think_token", {"text": chunk, "round": round_i + 1})
            except Exception as e:
                yield sse_event("error", {"detail": f"LLM stream error: {e}"})
                return
            ...
            if tool:
                ...
                yield sse_event(
                    "tool_call",
                    {"round": round_i + 1, "name": name, "arguments": args},
                )
                result = await self._run_tool(name, args)
                yield sse_event(
                    "tool_result",
                    {"name": name, "result_preview": str(result)[:1200]},
                )
            ...
            if CodeSafetyFilter.looks_like_code(final_text) and not CodeSafetyFilter.validate(
                final_text
            ):
            ...
            if not ok2:
                yield sse_event(
                    "error",
                    {"detail": "Final answer blocked by safety policy", "status": 403},
                )
                return
