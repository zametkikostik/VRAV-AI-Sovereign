"""Hardened code sandbox — AST allowlist, restricted exec, optional Docker/gVisor."""
...
class CodeSandbox:
    ...
    def _run_restricted(self, code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            results = {k: repr(v) for k, v in globals_dict.items()
                       if not k.startswith("_") and k not in safe_builtins and k != "__builtins__"}
            return {"ok": True, "stdout": stdout.getvalue()[:5000], "locals": results, "mode": "restricted"}
        except Exception as e:
            return {"ok": False, "error": str(e), "stdout": stdout.getvalue()[:2000], "mode": "restricted"}
    ...
    def _run_docker(self, code: str) -> Dict[str, Any]:
        with tempfile.TemporaryDirectory() as td:
            ...
            cmd = [
                ...
                "--user", "65534:65534", "-v", f"{script}:/code/main.py:ro",
            ]
            if self._has_runsc():
            ...
            try:
                r = subprocess.run(cmd, capture_output=True, text=True, timeout=self.timeout_sec + 8)
                return {"ok": r.returncode == 0, "stdout": (r.stdout or "")[:5000],
                        "stderr": (r.stderr or "")[:2000], "mode": mode, "returncode": r.returncode}
